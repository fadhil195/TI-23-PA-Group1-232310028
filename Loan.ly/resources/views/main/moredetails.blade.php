<!-- filepath: c:\laragon\www\Loan.ly\resources\views\main\moredetails.blade.php -->
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>{{ $provider->nama_penyedia }} - Details</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.7.2/font/bootstrap-icons.css" rel="stylesheet">
  <style>
    body {
      margin: 0;
      background: url('{{ asset('assets/img/Moredetails.png') }}') no-repeat center center fixed;
...
<body>
  ...
  <div class="top-bar">
    ...
    <a href="/home" class="btn-back px-4 py-2" style="border-radius:10px;">Back</a>
  </div>

  <!-- Card Content -->
  <div class="details-card">
<div class="details-header mb-3">
  <img src="{{ asset('assets/img/' . ($provider->logo ?? 'loan-logo.png')) }}" alt="{{ $provider->nama_penyedia }}" class="details-logo">
  <div>
    <div class="fw-bold" style="font-size:1.4rem;">{{ $provider->nama_penyedia }}</div>
    <div class="d-flex align-items-center mb-1">
      ...
      <span class="ms-2 fw-semibold" style="color:#222;">{{ $provider->rating }}</span>
    </div>
    <div style="color:#666;">{{ $provider->deskripsi }}</div>
  </div>
</div>
    ...
    <div class="details-meta">
      <div class="details-meta-item">
        <small>Est.APR</small>
        <span>{{ $provider->apr_min_estimasi }}% - {{ $provider->apr_max_estimasi }}%</span>
      </div>
      <div class="details-meta-item">
        <small>Min Credit Score</small>
        <span>{{ $provider->skor_kredit_min_estimasi }}</span>
      </div>
      <div class="details-meta-item">
    ...
    <div class="who-box mt-3">
      <h6>Who’s it for</h6>
      <p>
        Avant offers an extensive list of ways personal loans can be used besides the standard emergency, debt consolidation and home improvement scenarios. The site states an Avant personal loan can be used to fund adoption or infertility procedures, cover the cost of a wedding or vacation, or provide next-day funds for a business expense.
      </p>
    </div>
  </div>
</body>
